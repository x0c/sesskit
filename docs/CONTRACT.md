...
2. **Pi weekly rate limit:** `stopReason=error` + `errorMessage` → `STATUS_ABORTED`; error text retained in list fields and `load_events` (no longer user-only). Plain conversation hides error-only turns by default (chat view shows only real user/assistant text); pass `include_errors=True` (`sesskit show/export --include-errors`) to bring them back.
